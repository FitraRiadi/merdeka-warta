...
<head>
    ...
    @include('layouts.partials.seo-meta', [
        'title' => $announcement->title . ' | Merdeka Warta',
        'description' => Str::limit($announcement->content_text, 160),
        'type' => 'article',
        'publishedTime' => $announcement->created_at?->toIso8601String(),
    ])
</head>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <article class="lg:col-span-2 flex flex-col gap-6 bg-paper p-6 md:p-8 rounded-xl bento-card">
                ...
                <div class="flex flex-wrap gap-2 pt-6 border-t border-outline-variant">
                    @foreach($tags as $tag)
                    <a class="bg-surface-container-high rounded px-3 py-1 font-label-mono text-[10px] hover:bg-primary hover:text-on-primary transition-all bento-shadow" href="{{ route('public.announcement.list', ['tag' => substr($tag, 1)]) }}#announcements-section">{{ $tag }}</a>
                    @endforeach
                </div>
                @endif
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-5">
                        ...
                        <div class="bg-white dark:bg-surface-container rounded-xl bento-shadow bento-card bento-shadow-hover flex flex-col overflow-hidden">
                            <div class="p-4 flex flex-col flex-grow">
                                ...
                                <h3 class="font-headline-lg text-base uppercase mb-1.5 leading-tight line-clamp-2">{{ $other->title }}</h3>
