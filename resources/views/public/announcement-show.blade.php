        ...
        <div class="bento-grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
            ...
            <article class="lg:col-span-8 flex flex-col gap-6">
                ...
                <div class="flex items-center gap-3 flex-wrap">
                    ...
                    @endphp
                    <span class="{{ $typeColor }} px-3 py-1 rounded-lg font-label-mono text-[10px] uppercase flex items-center gap-1.5">
                ...
                <section class="mt-8 pt-8 border-t border-outline-variant">
                    ...
                    <div class="bento-grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <a href="{{ route('public.announcement.show', $other->id) }}" class="bg-white rounded-2xl bento-shadow bento-card bento-shadow-hover p-4 md:p-5 group">
                            <div class="flex items-center gap-2 mb-2">
                                @php
                                    $oColor = $typeColors[$other->type] ?? 'bg-secondary/10 text-secondary';
                                    $oLabel = $typeLabels[$other->type] ?? 'PENGUMUMAN';
                                @endphp
                                <span class="{{ $oColor }} px-2 py-0.5 text-[10px] font-label-mono uppercase rounded-lg">{{ $oLabel }}</span>
            ...
            <aside class="lg:col-span-4 flex flex-col gap-6">
                ...
                <section class="bg-white rounded-2xl bento-shadow p-5 md:p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <a href="{{ route('public.announcement.show', $other->id) }}" class="group cursor-pointer block bento-card rounded-xl p-2 -mx-2 hover:bg-surface-container-low">
                            @php
                                $oColor = $typeColors[$other->type] ?? 'bg-secondary/10 text-secondary';
                                $oLabel = $typeLabels[$other->type] ?? 'PENGUMUMAN';
                            @endphp
                            <span class="text-[10px] font-label-mono {{ $oColor }} px-2 py-0.5 rounded-lg inline-block mb-1">{{ $oLabel }}</span>
