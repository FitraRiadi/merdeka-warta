        ...
        <section class="mb-12">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
                ...
                <form method="GET" action="{{ route('public.article.list') }}" class="relative w-full md:w-72" id="searchForm">
                    ...
                    @endif
                    <input name="search" class="bg-white bento-shadow rounded-xl px-4 py-2.5 font-label-mono focus:outline-none focus:ring-2 focus:ring-primary/30 w-full text-sm uppercase" placeholder="SEARCH ARTICLE..." type="text" value="{{ request('search') }}">
                ...
                @foreach($articles as $index => $article)
                    <article class="bg-white rounded-2xl bento-shadow bento-card bento-shadow-hover flex flex-col overflow-hidden group">
                        ...
                        <div class="p-3 md:p-4 flex flex-col flex-grow">
                            ...
                            <h3 class="font-headline-lg text-sm md:text-base uppercase leading-tight mb-1.5 line-clamp-2">{{ $article->title }}</h3>
            ...
            <div class="mt-12 flex justify-center items-center gap-2">
                ...
                @else
                    <a href="{{ $articles->previousPageUrl() . '#articles-section' }}" class="w-10 h-10 flex items-center justify-center bg-white rounded-xl bento-shadow hover:bg-primary hover:text-on-primary transition-all bento-card">
                        <span class="material-symbols-outlined">chevron_left</span>
                    </a>
                @endif
                ...
                <div class="flex gap-1.5">
                    @foreach($pageRange as $page)
                        ...
                        @else
                            <a href="{{ $url }}" class="w-10 h-10 flex items-center justify-center bg-white font-label-mono rounded-xl hover:bg-surface-container-high transition-colors bento-shadow">{{ $page }}</a>
                        @endif
                    @endforeach
                </div>

                @if($articles->hasMorePages())
                    <a href="{{ $articles->nextPageUrl() . '#articles-section' }}" class="w-10 h-10 flex items-center justify-center bg-white rounded-xl bento-shadow hover:bg-primary hover:text-on-primary transition-all bento-card">
