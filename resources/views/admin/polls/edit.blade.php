...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="font-headline-lg text-2xl md:text-3xl uppercase">Edit Polling</h1>
    </div>

    <form method="POST" action="{{ route('admin.polls.update', $poll) }}"
          x-data="pollForm({{ json_encode([
              'type' => $poll->type,
              'mode' => $poll->mode,
              'options' => $poll->options->map(fn($o) => ['text' => $o->option_text, 'is_correct' => $o->is_correct])->toArray(),
          ]) }})"
          class="bg-white dark:bg-surface-container rounded-xl bento-shadow p-6 md:p-8 border-2 border-black dark:border-gray-700">
        @csrf @method('PATCH')
        ...
        <div class="mb-6">
            ...
            <input type="datetime-local" name="closes_at" value="{{ old('closes_at', $poll->closes_at ? $poll->closes_at->format('Y-m-d\TH:i') : '') }}"
                   class="w-full bg-surface-container-low rounded-xl px-4 py-3 font-bold text-sm border-2 border-black dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-primary/30">
        </div>
        ...
        <div class="mb-6">
            ...
            <p class="font-label-mono text-[10px] text-on-surface-variant/60 mb-3">Perhatian: mengubah opsi akan menghapus semua voting yang sudah masuk.</p>
        ...
        <div class="mb-8">
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ $poll->is_active ? 'checked' : '' }} class="w-4 h-4">
                <span class="font-label-mono text-[10px] uppercase">Aktif</span>
            </label>
        </div>
...
@push('scripts')
<script>
    function pollForm(initial) {
        return {
            form: initial || { type: 'single', mode: 'biasa', options: [{ text: '', is_correct: false }, { text: '', is_correct: false }] },
            addOption() {
